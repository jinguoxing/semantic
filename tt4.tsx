...
const mockAICandidates = [
    ...
    },
    {
        ...
        suggestedName: '新生儿 (Newborn)', // Conflict scenario
        confidence: 0.78,
...
const AssetScanningView = ({ setActiveModule, candidates }) => {
    ...
    const handleGenerateCandidates = () => { if (selectedTables.length === 0) { alert("请先选择至少一个物理表进行分析。"); return; } alert(`正在对 ${selectedTables.length} 个表进行 AI 语义分析...`); setActiveModule('bu_candidates'); };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                ...
                <table className="w-full text-sm text-left"><thead className="bg-slate-50 border-b"><tr><th className="px-6 py-3 w-10"><input type="checkbox" /></th><th className="px-6 py-3">物理表名</th><th className="px-6 py-3">说明</th><th className="px-6 py-3">状态</th><th className="px-6 py-3 text-right">操作</th></tr></thead>
                    <tbody>{mockScanAssets.map(a => (<tr key={a.id} className="border-b"><td className="px-6 py-4"><input type="checkbox" checked={selectedTables.includes(a.name)} onChange={() => setSelectedTables(prev => prev.includes(a.name) ? prev.filter(n => n !== a.name) : [...prev, a.name])} /></td><td className="px-6 py-4 font-mono">{a.name}</td><td className="px-6 py-4">{a.comment}</td><td className="px-6 py-4"><span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs">{a.status}</span></td><td className="px-6 py-4 text-right"><button className="text-purple-600 hover:text-purple-800 text-xs font-medium mr-2" onClick={() => { setSelectedTables([a.name]); handleGenerateCandidates(); }}>生成</button></td></tr>))}</tbody>
                </table>
            </div>
        </div>
    );
...
const CandidateGenerationView = ({ businessObjects, setBusinessObjects, candidates, setCandidates }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center"><h2 className="text-2xl font-bold text-slate-800">智能候选生成</h2></div>
            <div className="grid grid-cols-3 gap-6">{candidates.map(c => {
            ...
            {viewingCandidate && (
                <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl p-6 w-full max-w-lg space-y-4">
                        <h3 className="font-bold text-lg">候选详情</h3>
                        <div><label className="block text-sm font-bold text-slate-500">源表</label><div className="p-2 bg-slate-50 rounded text-sm font-mono">{viewingCandidate.sourceTable}</div></div>
                        <div><label className="block text-sm font-bold text-purple-600">对象名称</label><input type="text" value={editName} onChange={(e) => handleNameChange(e.target.value)} className={`w-full border p-2 rounded ${nameError ? 'border-red-500' : ''}`} />{nameError && <div className="text-red-500 text-xs mt-1">{nameError}</div>}</div>
                        <div className="flex justify-between pt-4"><button onClick={() => handleReject(viewingCandidate.id)} className="text-red-600 text-sm">拒绝</button><div className="flex gap-2"><button onClick={() => setViewingCandidate(null)} className="px-4 py-2 text-slate-600">取消</button><button onClick={handleConfirmAccept} disabled={!!nameError} className={`px-4 py-2 text-white rounded ${nameError ? 'bg-purple-300' : 'bg-purple-600'}`}>确认创建</button></div></div>
                    </div>
                </div>
            )}
        </div>
