...
### 阶段一：框架与路由
- [ ] 新建 `SemanticVersionView`（列表+详情骨架）
- [ ] `App.tsx` 增加 `semantic_version` 路由
- [ ] Header/Sidebar 文案与入口补齐

### 阶段二：对象快照（复用）
- [ ] `BusinessModelingView` 增加 `readOnly` 适配
- [ ] 版本态隐藏/禁用所有编辑能力
- [ ] 版本快照标识组件（ReadonlyBadge）
